...
<body>
  ...
  <?php include 'top.php'; ?>
   ...
   <section class="breadcrumb breadcrumb_bg">
    ...
    if($searchType == "subject"){
      $sql = "SELECT * FROM board_bigmuscle7 WHERE category = '$bigMuscle' AND subject LIKE '%$keyword%' ORDER BY no DESC LIMIT $pageNo_tmp, 5";
      // $sql_page_count = "SELECT COUNT(*) FROM board_bigmuscle7 WHERE category = '$bigMuscle' AND subject LIKE '%$keyword%' ORDER BY no DESC LIMIT $pageNo_tmp, 5";
      $sql_page_count = "SELECT COUNT(*) FROM board_bigmuscle7 WHERE category = '$bigMuscle' AND subject LIKE '%$keyword%' ORDER BY no DESC";
    }else if($searchType == "contents"){
      $sql = "SELECT * FROM board_bigmuscle7 WHERE category = '$bigMuscle' AND contents LIKE '%$keyword%' ORDER BY no DESC LIMIT $pageNo_tmp, 5";
      // $sql_page_count = "SELECT COUNT(*) FROM board_bigmuscle7 WHERE category = '$bigMuscle' AND contents LIKE '%$keyword%' ORDER BY no DESC LIMIT $pageNo_tmp, 5";
      $sql_page_count = "SELECT COUNT(*) FROM board_bigmuscle7 WHERE category = '$bigMuscle' AND contents LIKE '%$keyword%' ORDER BY no DESC";
    }else if($searchType == "writter"){
      $sql = "SELECT * FROM board_bigmuscle7 WHERE category = '$bigMuscle' AND writter = '$keyword' ORDER BY no DESC LIMIT $pageNo_tmp, 5";
      // $sql_page_count = "SELECT COUNT(*) FROM board_bigmuscle7 WHERE category = '$bigMuscle' AND writter = '$keyword' ORDER BY no DESC LIMIT $pageNo_tmp, 5";
      $sql_page_count = "SELECT COUNT(*) FROM board_bigmuscle7 WHERE category = '$bigMuscle' AND writter = '$keyword' ORDER BY no DESC";
    }else{
      //아무것도 아닌것이 넘어
    }

    //마이페이지로부터 해당 페이지에 오면 bigMuscle변수가 없다. 따라서 이에 대한 처리를 해준다.
    if(empty($bigMuscle) && $searchType == "writter"){
      //만약 bigMuscle변수가 없다면 다른 쿼리문을 정의해준다.
      $sql = "SELECT * FROM board_bigmuscle7 WHERE writter = '$keyword' ORDER BY no DESC LIMIT $pageNo_tmp, 5";
      $sql_page_count = "SELECT COUNT(*) FROM board_bigmuscle7 WHERE writter = '$keyword' ORDER BY no DESC LIMIT $pageNo_tmp, 5";
    }

    //GET방식으로 받아온 특정 빅머슬 카테고리, 특정 페이지 번호인 게시물 튜플들을 가져온다.
    // $sql = "SELECT * FROM board_bigmuscle7 WHERE category = '$bigMuscle' ORDER BY no DESC LIMIT $pageNo_tmp, 5";
                  ...
                  <?php
                    ...
                    <tr class="row100 body">
                      ...
                      if($total_reply_count == 0 ){
                        ...
                        $category = '';
                        if($row['category'] == "squart"){
                          $category = "스쿼트";
                        }else if($row['category'] == "deadlift"){
                          $category = "데드리프트";
                        }else if($row['category'] == "benchpress"){
                          $category = "벤치프레스";
                        }else if($row['category'] == "dips"){
                          $category = "딥스";
                        }else if($row['category'] == "militarypress"){
                          $category = "밀리터리프레스";
                        }else if($row['category'] == "barbelrow"){
                          $category = "바벨로우";
                        }else{
                          $category = "턱걸이";
                        }
                        ?>
                        <td class="cell100 column2"><a href = 'board.php?no=<?php echo $row['no'] ?>&bigMuscle7=<?php echo $row['category'] ?>'>[<?php echo $category ?>]&nbsp;<?php echo $row['subject']; ?> </a></td>
                        <?php
                      }else{
                        //그게 아니라면 ( = 댓글수가 0개가 아니라면 = 1개 이상이라면 )댓글 수를 표시해준다.
                        ?>
                        <td class="cell100 column2"><a href = 'board.php?no=<?php echo $row['no'] ?>&bigMuscle7=<?php echo $row['category'] ?>'>[<?php echo $category ?>]&nbsp;<?php echo $row['subject']; ?>...[<?php echo $total_reply_count ?>] </a></td>
                        <?php
                      }
                       ?>
  ...
  <form action="boardInsert.php" method="post" style = "float : right">
    <input type="hidden" name="bigMuscle7" value="<?php echo "$bigMuscle" ?>">
  ...
  <div class="box-footer">
  	<div class="text-center">
        ...
        if($pageNo == 1){
          ...
          ?>
          <li><a href="searchBoardList.php?bigMuscle7=<?php echo"$bigMuscle"; ?>&pageNo=<?php echo$pageNo-1; ?>&searchType=<?php echo"$searchType" ?>&keyword=<?php echo"$keyword" ?>">이전</a></li>
          <?php
        }
         ?>
  				<li >
          ...
          // $sql_page_count = "SELECT COUNT(*) FROM board_bigmuscle7 WHERE category = '$bigMuscle'";
          //마이페이지로부터 해당 페이지에 오면 bigMuscle변수가 없다. 따라서 이에 대한 처리를 해준다.
          if(empty($bigMuscle) && $searchType == "writter"){
            $nickname = $_GET['keyword'];
            // var_dump("멈춤");exit;
              $sql_page_count = "SELECT COUNT(*) FROM board_bigmuscle7 WHERE writter = '$nickname'";
          }
            ...
            $endPageNo = $pageNum;
                ...
                // for($i = $startPageNo ; $i <= $total_page_count/$one_page_count + ($total_page_count%$one_page_count == 0 ? 0 : 1) ; $i ++){
                  if($pageNo == $i){
                    ...
                    ?>
                    <a style = "background : #4371B3; color : white;" href="searchBoardList.php?bigMuscle7=<?php echo"$bigMuscle"; ?>&pageNo=<?php echo"$i"; ?>&searchType=<?php echo"$searchType" ?>&keyword=<?php echo"$keyword" ?>"><?php echo "$i"; ?></a>
                    <?php
                  }else{
                    ?>
                    <a href="searchBoardList.php?bigMuscle7=<?php echo"$bigMuscle"; ?>&pageNo=<?php echo"$i"; ?>&searchType=<?php echo"$searchType" ?>&keyword=<?php echo"$keyword" ?>"><?php echo "$i"; ?></a>
                    <?php
                  }
                }
          ?>
          ...
          if($totalBlock == $nowBlock && $pageNo == $endPageNo){
            ...
            ?>
            <li> <a href="searchBoardList.php?bigMuscle7=<?php echo"$bigMuscle"; ?>&pageNo=<?php echo$pageNo+1; ?>&searchType=<?php echo"$searchType" ?>&keyword=<?php echo"$keyword" ?>">다음</a></li>
            <?php
          }
           ?>
  		</ul>
  ...
  <!--게시판 번호-->
  <form action="searchBoardList.php" method="get">
    <div class="box-footer">
