...
<body>
  ...
  <?php include 'top.php'; ?>
   ...
   <section class="breadcrumb breadcrumb_bg">
    ...
    if($searchType == "subject"){
      $sql = "SELECT * FROM board_injury WHERE category = '$injury' AND subject LIKE '%$keyword%' ORDER BY no DESC LIMIT $pageNo_tmp, 5";
      $sql_page_count = "SELECT COUNT(*) FROM board_injury WHERE category = '$injury' AND subject LIKE '%$keyword%' ORDER BY no DESC";
    }else if($searchType == "contents"){
      // var_dump($injury);
      // var_dump($keyword);
      // exit;
      $sql = "SELECT * FROM board_injury WHERE category = '$injury' AND contents LIKE '%$keyword%' ORDER BY no DESC LIMIT $pageNo_tmp, 5";
      $sql_page_count = "SELECT COUNT(*) FROM board_injury WHERE category = '$injury' AND contents LIKE '%$keyword%' ORDER BY no DESC";
    }else if($searchType == "writter"){
      $sql = "SELECT * FROM board_injury WHERE category = '$injury' AND writter = '$keyword' ORDER BY no DESC LIMIT $pageNo_tmp, 5";
      $sql_page_count = "SELECT COUNT(*) FROM board_injury WHERE category = '$injury' AND writter = '$keyword' ORDER BY no DESC LIMIT";
    }else{
      //아무것도 아닌것이 넘어
    }

    //마이페이지로부터 해당 페이지에 오면 injury 없다. 따라서 이에 대한 처리를 해준다.
    if(empty($injury) && $searchType == "writter"){
      //만약 injury변수가 없다면 다른 쿼리문을 정의해준다.
      $sql = "SELECT * FROM board_injury WHERE writter = '$keyword' ORDER BY no DESC LIMIT $pageNo_tmp, 5";
      $sql_page_count = "SELECT COUNT(*) FROM board_injury WHERE writter = '$keyword' ORDER BY no DESC LIMIT $pageNo_tmp, 5";
    }

    //GET방식으로 받아온 특정 빅머슬 카테고리, 특정 페이지 번호인 게시물 튜플들을 가져온다.
    // $sql = "SELECT * FROM board_injury WHERE category = '$injury' ORDER BY no DESC LIMIT $pageNo_tmp, 5";
                  ...
                  <?php
                    ...
                    <tr class="row100 body">
                      ...
                      if($total_reply_count == 0 ){
                        ...
                        $category = '';
                        if($row['category'] == "neck"){
                          $category = "목&어깨";
                        }else if($row['category'] == "back"){
                          $category = "허리";
                        }else if($row['category'] == "leg"){
                          $category = "무릎&발목";
                        }else{
                          $category = "팔꿈치&손목";
                        }
                        ?>
                        <td class="cell100 column2"><a href = 'board_injury.php?no=<?php echo $row['no'] ?>&injury=<?php echo $row['category'] ?>'>[<?php echo $category ?>]&nbsp;<?php echo $row['subject']; ?> </a></td>
                        <?php
                      }else{
                        //그게 아니라면 ( = 댓글수가 0개가 아니라면 = 1개 이상이라면 )댓글 수를 표시해준다.
                        ?>
                        <td class="cell100 column2"><a href = 'board_injury.php?no=<?php echo $row['no'] ?>&injury=<?php echo $row['category'] ?>'>[<?php echo $category ?>]&nbsp;<?php echo $row['subject']; ?>...[<?php echo $total_reply_count ?>] </a></td>
                        <?php
                      }
                       ?>
  ...
  <form action="boardInsert_injury.php" method="post" style = "float : right">
  ...
  <div class="box-footer">
  	<div class="text-center">
        ...
        if($pageNo == 1){
          ...
          ?>
          <li><a href="searchBoardList_injury.php?injury=<?php echo"$injury"; ?>&pageNo=<?php echo$pageNo-1; ?>&searchType=<?php echo"$searchType" ?>&keyword=<?php echo"$keyword" ?>">이전</a></li>
          <?php
        }
         ?>
  				<li >
          <?php
          $one_page_count = 5;
          //$sql_page_count = "SELECT COUNT(*) FROM board_injury WHERE category = '$injury'";
          if(empty($injury) && $searchType == "writter"){
            $nickname = $_GET['keyword'];
            $sql_page_count = "SELECT COUNT(*) FROM board_injury WHERE writter = '$nickname'";
          }
            ...
            $endPageNo = $pageNum;
                ...
                // for($i = $startPageNo ; $i <= $total_page_count/$one_page_count + ($total_page_count%$one_page_count == 0 ? 0 : 1) ; $i ++){
                  if($pageNo == $i){
                    ...
                    ?>
                    <a style = "background : #4371B3; color : white;" href="searchBoardList_injury.php?injury=<?php echo"$injury"; ?>&pageNo=<?php echo"$i"; ?>&searchType=<?php echo"$searchType" ?>&keyword=<?php echo"$keyword" ?>"><?php echo "$i"; ?></a>
                    <?php
                  }else{
                    ?>
                    <a href="searchBoardList_injury.php?injury=<?php echo"$injury"; ?>&pageNo=<?php echo"$i"; ?>&searchType=<?php echo"$searchType" ?>&keyword=<?php echo"$keyword" ?>"><?php echo "$i"; ?></a>
                    <?php
                  }
                }
          ?>
          ...
          if($totalBlock == $nowBlock && $pageNo == $endPageNo){
            ...
            ?>
            <li> <a href="searchBoardList_injury.php?injury=<?php echo"$injury"; ?>&pageNo=<?php echo$pageNo+1; ?>&searchType=<?php echo"$searchType" ?>&keyword=<?php echo"$keyword" ?>">다음</a></li>
            <?php
          }
           ?>
  		</ul>
  ...
  <!--게시판 번호-->
  <form action="searchBoardList_injury.php" method="get">
    <div class="box-footer">
