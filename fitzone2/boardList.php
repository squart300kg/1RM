...
<body>
                  ...
                  <?php
                    ...
                    <tr class="row100 body">
                      ...
                      if($total_reply_count == 0 ){
                        ...
                        ?>
                        <td class="cell100 column2"><a href = 'board.php?no=<?php echo $row['no'] ?>&bigMuscle7=<?php echo $row['category'] ?>'><?php echo $row['subject']; ?> </a></td>
                        <?php
                      }else{
                        //그게 아니라면 ( = 댓글수가 0개가 아니라면 = 1개 이상이라면 )댓글 수를 표시해준다.
                        ?>
                        <td class="cell100 column2"><a href = 'board.php?no=<?php echo $row['no'] ?>&bigMuscle7=<?php echo $row['category'] ?>'><?php echo $row['subject']; ?>...[<?php echo $total_reply_count ?>] </a></td>
                        <?php
                      }
                       ?>
  ...
  <form action="boardInsert.php" method="post" style = "float : right" name = "boardInsertForm" id = "boardInsertForm">
    <input type="hidden" name="bigMuscle7" value="<?php echo "$bigMuscle" ?>">
    <button type="button" onclick = "boardInsert('<?php echo "$id" ?>')" class="button button-contactForm btn_2" >글쓰기</button>
  </form>
  <!--게시판 번호-->
  ...
  <div class="box-footer">
  	<div class="text-center">
        ...
        if($pageNo == 1){
          ...
          ?>
          <li><a href="boardList.php?bigMuscle7=<?php echo"$bigMuscle"; ?>&pageNo=<?php echo$pageNo-1; ?>">이전</a></li>
          <?php
        }
         ?>
  				<li >
            ...
            $endPageNo = $pageNum;
                ...
                // for($i = $startPageNo ; $i <= $total_page_count/$one_page_count + ($total_page_count%$one_page_count == 0 ? 0 : 1) ; $i ++){
                  if($pageNo == $i){
                    ...
                    ?>
                    <a style = "background : #4371B3; color : white;" href="boardList.php?bigMuscle7=<?php echo"$bigMuscle"; ?>&pageNo=<?php echo"$i"; ?>"><?php echo "$i"; ?></a>
                    <?php
                  }else{
                    ?>
                    <a href="boardList.php?bigMuscle7=<?php echo"$bigMuscle"; ?>&pageNo=<?php echo"$i"; ?>"><?php echo "$i"; ?></a>
                    <?php
                  }
                }
          ?>
          ...
          if($totalBlock == $nowBlock && $pageNo == $endPageNo){
            ...
            ?>
            <li> <a href="boardList.php?bigMuscle7=<?php echo"$bigMuscle"; ?>&pageNo=<?php echo$pageNo+1; ?>">다음</a></li>
            <?php
          }
           ?>
  ...
  <?php
  // var_dump($startPageNo);exit;
  // exit;
  // var_dump("전체 블럭의 수 : " + $totalBlock);
  // var_dump("지금 현재 있는 블럭 인덱스 : " + $nowBlock);
  // var_dump("페이지 번호 : " + $pageNo);
  // var_dump("끝 페이지 번호 : " + $endPageNo);
  // exit;
   ?>
