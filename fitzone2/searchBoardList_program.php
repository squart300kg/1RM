...
<body>
  ...
  <?php include 'top.php'; ?>
   ...
   <section class="breadcrumb breadcrumb_bg">
    ...
    if($searchType == "subject"){
      $sql = "SELECT * FROM board_program WHERE category = '$program' AND subject LIKE '%$keyword%' ORDER BY no DESC LIMIT $pageNo_tmp, 5";
      $sql_page_count = "SELECT COUNT(*) FROM board_program WHERE category = '$program' AND subject LIKE '%$keyword%' ORDER BY no DESC";
    }else if($searchType == "contents"){
      // var_dump($program);
      // var_dump($keyword);
      // exit;
      $sql = "SELECT * FROM board_program WHERE category = '$program' AND contents LIKE '%$keyword%' ORDER BY no DESC LIMIT $pageNo_tmp, 5";
      $sql_page_count = "SELECT COUNT(*) FROM board_program WHERE category = '$program' AND contents LIKE '%$keyword%' ORDER BY no DESC";
    }else if($searchType == "writter"){
      $sql = "SELECT * FROM board_program WHERE category = '$program' AND writter = '$keyword' ORDER BY no DESC LIMIT $pageNo_tmp, 5";
      $sql_page_count = "SELECT COUNT(*) FROM board_program WHERE category = '$program' AND writter = '$keyword' ORDER BY no DESC";
    }else{
      //아무것도 아닌것이 넘어
    }

    if(empty($program) && $searchType == "writter"){
      //만약 bigMuscle변수가 없다면 다른 쿼리문을 정의해준다.
      $sql = "SELECT * FROM board_program WHERE writter = '$keyword' ORDER BY no DESC LIMIT $pageNo_tmp, 5";
      $sql_page_count = "SELECT COUNT(*) FROM board_program WHERE writter = '$keyword' ORDER BY no DESC LIMIT $pageNo_tmp, 5";
    }

    //GET방식으로 받아온 특정 빅머슬 카테고리, 특정 페이지 번호인 게시물 튜플들을 가져온다.
    // $sql = "SELECT * FROM board_program WHERE category = '$program' ORDER BY no DESC LIMIT $pageNo_tmp, 5";
                  ...
                  <?php
                    ...
                    <tr class="row100 body">
                      ...
                      if($total_reply_count == 0 ){
                        ...
                        if($row['category'] == "300"){
                          $category = "For 3대 ~300";
                        }else if($row['category'] == "400"){
                          $category = "For 3대 400";
                        }else if($row['category'] == "500"){
                          $category = "For 3대 500";
                        }else if($row['category'] == "600"){
                          $category = "For 3대 600";
                        }
                        ?>
                        <td class="cell100 column2"><a href = 'board_program.php?no=<?php echo $row['no'] ?>&program=<?php echo $row['category'] ?>'>[<?php echo $category ?>]&nbsp;<?php echo $row['subject']; ?> </a></td>
                        <?php
                      }else{
                        //그게 아니라면 ( = 댓글수가 0개가 아니라면 = 1개 이상이라면 )댓글 수를 표시해준다.
                        ?>
                        <td class="cell100 column2"><a href = 'board_program.php?no=<?php echo $row['no'] ?>&program=<?php echo $row['category'] ?>'>[<?php echo $category ?>]&nbsp;<?php echo $row['subject']; ?>...[<?php echo $total_reply_count ?>] </a></td>
                        <?php
                      }
                       ?>
  ...
  <form action="boardInsert_program.php" method="post" style = "float : right">
  ...
  <div class="box-footer">
  	<div class="text-center">
        ...
        if($pageNo == 1){
          ...
          ?>
          <li><a href="searchBoardList_program.php?program=<?php echo"$program"; ?>&pageNo=<?php echo$pageNo-1; ?>&keyword=<?php echo"$keyword" ?>&searchType=<?php echo"$searchType" ?>">이전</a></li>
          <?php
        }
         ?>
  				<li >
          <?php
          $one_page_count = 5;
          //$sql_page_count = "SELECT COUNT(*) FROM board_program WHERE category = '$program'";
          if(empty($bigMuscle) && $searchType == "writter"){
            $nickname = $_GET['keyword'];
              $sql_page_count = "SELECT COUNT(*) FROM board_program WHERE writter = '$nickname'";
          }
            ...
            $endPageNo = $pageNum;
                ...
                // for($i = $startPageNo ; $i <= $total_page_count/$one_page_count + ($total_page_count%$one_page_count == 0 ? 0 : 1) ; $i ++){
                  if($pageNo == $i){
                    ...
                    ?>
                    <a style = "background : #4371B3; color : white;" href="searchBoardList_program.php?program=<?php echo"$program"; ?>&pageNo=<?php echo"$i"; ?>&keyword=<?php echo"$keyword" ?>&searchType=<?php echo"$searchType" ?>"><?php echo "$i"; ?></a>
                    <?php
                  }else{
                    ?>
                    <a href="searchBoardList_program.php?program=<?php echo"$program"; ?>&pageNo=<?php echo"$i"; ?>&keyword=<?php echo"$keyword" ?>&searchType=<?php echo"$searchType" ?>"><?php echo "$i"; ?></a>
                    <?php
                  }
                }
          ?>
          ...
          if($totalBlock == $nowBlock && $pageNo == $endPageNo){
            ...
            ?>
            <li> <a href="searchBoardList_program.php?program=<?php echo"$program"; ?>&pageNo=<?php echo$pageNo+1; ?>&keyword=<?php echo"$keyword" ?>&searchType=<?php echo"$searchType" ?>">다음</a></li>
            <?php
          }
           ?>
  ...
  <!--게시판 번호-->
  <form action="searchBoardList_program.php" method="get">
    <div class="box-footer">
